...
print "Begin parsing lines. Time: " + str(time.time() - cur_time)
cur_time = time.time()
for l in f:
	if l[0] in ('w', 'W', '%'): continue
	if 'X:' in l:
                print "X, Time: " + str(time.time() - cur_time)
                cur_time = time.time()
		sn = int(l.split(':')[1])
		if sn == num:
			sel = True
	if 'L:' in l and sel:
                print "L, Time: " + str(time.time() - cur_time)
                cur_time = time.time()
		nunit = l.split(':')[1].strip()
		unit = int(l.split('/')[1])
	if 'M:' in l and sel:
                print "M, Time: " + str(time.time() - cur_time)
                cur_time = time.time()
		meter = l.split(':')[1].strip()
		if meter == 'C': meter = "4/4"
	if 'Q:' in l and sel:
                print "Q, Time: " + str(time.time() - cur_time)
                cur_time = time.time()
		bpm = get_bpm(l.split(':')[1].strip(), nunit)
	if 'K:' in l and sel:
                print "K, Time: " + str(time.time() - cur_time)
                cur_time = time.time()
